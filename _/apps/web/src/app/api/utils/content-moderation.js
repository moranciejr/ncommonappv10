...
const SUSPICIOUS_PATTERNS = [
  ...
  /\b(scam|fraud|money\s*transfer)\b/i,
...
export function moderateContent(text, options = {}) {
  ...
  if (matchesSuspiciousPatterns(text)) {
    ...
    if (/\b(drug|weed|cocaine|heroin|meth)\b/i.test(text)) {
      return {
        allowed: false,
        filtered: text,
    ...
    if (/\b(kill|murder|suicide|die)\b/i.test(text)) {
      return {
        allowed: false,
        filtered: text,
    ...
    if (/\b(hate|racist|nazi)\b/i.test(text)) {
      return {
        allowed: false,
        filtered: text,
        reason: "hate_speech",
