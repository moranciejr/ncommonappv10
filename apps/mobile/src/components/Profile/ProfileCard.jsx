...
function Pill({ text, tone = "default" }) {
  ...
  const borderColor = tone === "urgent" ? colors.yellow : colors.border;
...
export default function ProfileCard({
  ...
  const heroBadge = useMemo(() => {
    ...
    const bg = isUrgent ? colors.yellow : "rgba(26,31,46,0.88)";
    const borderColor = isUrgent ? colors.yellow : colors.border;
    const color = isUrgent ? "#000" : colors.text;
      ...
      <View style={{ padding: 14 }}>
        ...
        {timeLine ? (
          <Text
            style={{
              marginTop: 8,
              color: startState?.isUrgent ? colors.yellow : colors.subtext,
              fontWeight: "900",
            }}
            numberOfLines={3}
          >
