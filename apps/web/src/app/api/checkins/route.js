...
export async function POST(request) {
  try {
    ...
    const latRaw = typeof body?.lat === "number" ? body.lat : null;
    const lngRaw = typeof body?.lng === "number" ? body.lng : null;

    // Validate coordinate ranges — reject impossible values before storing.
    const lat =
      latRaw !== null && Number.isFinite(latRaw) && latRaw >= -90 && latRaw <= 90
        ? latRaw
        : null;
    const lng =
      lngRaw !== null && Number.isFinite(lngRaw) && lngRaw >= -180 && lngRaw <= 180
        ? lngRaw
        : null;
