...
export async function POST(request) {
  try {
    ...
    // Rate limit: 3 requests per hour per email (use email as key, safe to expose)
    const emailKey = `pw_reset:${email.toLowerCase().trim()}`.slice(0, 128);
    const ok = await consumeRateLimit(sql, {
      userId: emailKey,
      action: "password_reset_request_per_hour",
      windowSeconds: 60 * 60,
      limit: 3,
    });
    if (!ok) {
