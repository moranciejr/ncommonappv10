...
// Allow users to mark habits for today or yesterday only (prevents unlimited backdating).
function clampToggleDate(isoDate) {
  const today = new Date().toISOString().slice(0, 10);
  const yesterday = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
  if (isoDate === today || isoDate === yesterday) {
    return isoDate;
  }
  return today;
}
...
export async function POST(request, { params }) {
  try {
    ...
    const date = clampToggleDate(toIsoDateOnly(body?.date) || todayIsoDate());

    // Basic rate limit — prevents spam toggling.
    const okRate = await consumeRateLimit(sql, {
      userId,
      action: "habit_toggle_per_minute",
      windowSeconds: 60,
      limit: 30,
    });
    if (!okRate) {
