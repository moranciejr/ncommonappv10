...
export async function POST(request, { params }) {
  try {
    ...
    // Idempotency: don't re-process a request that's already been resolved
    if (row.status !== "pending") {
      return Response.json(
        { ok: true, status: row.status, alreadyResolved: true },
        { status: 200 },
      );
    }

    // Rate limit: 30 responds/hour per user
    const withinLimit = await consumeRateLimit(sql, {
      userId,
      action: "checkin_request_respond",
      windowSeconds: 3600,
      limit: 30,
    });
    if (!withinLimit) {
